_G.LibLinq_1_0_Loader(function(LibLinq)
	...
	function Queue.prototype:VerifyContract()
		...
		while true do
			...
			if not contract(table[index]) then
				error(("Element %s does not meet the contract for this Queue."):format(tostring2(table[index])), 2)
			end
		end
	end
	...
	function Queue.prototype:Enqueue(item)
		...
		if contract and not contract(item) then
			error(("Element %s does not meet the contract for this Queue."):format(tostring2(item)), 2)
		end
